...
import (
	...
	"github.com/go-kratos/kratos/v2/contrib/authz"
...
func TestCasbin(t *testing.T) {
	...
	m, _ := model.NewModelFromFile("./authz_model.conf")
	//m.GetLogger().EnableLog(true)
	//m.PrintModel()
	//m.PrintPolicy()

	a := fileAdapter.NewAdapter("./authz_policy.csv")
...
func TestCasbin1(t *testing.T) {
	enforcer, err := casbin.NewSyncedEnforcer("./authz_model.conf", "./authz_policy.csv")
	if err != nil {
		panic(err)
	}
	enforcer.EnableEnforce(true)
